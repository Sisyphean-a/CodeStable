...
`package:skills` 是 CodeStable 的核心产品。它是一套以项目理解为核心、由开发者请求驱动的 AI 编程技能：开发者可以提出编程任务、项目问题或探索请求，即使并不了解项目当前状态；技能负责理解相关代码事实和开发者需求，并完成相应工作。开发者想进一步了解时，可以按自己的意愿查看、追问和学习项目，而不必把掌握实现细节作为使用前提。

- 项目理解是技能包的首要目标，编程任务是主要场景。
- AI 需要理解完成请求所需的真实项目，以及开发者提出请求的需求、意图和约束。
- 开发者可以只掌握项目的大致用途和自己的目标；当前状态、具体调用和实现细节由 AI 按请求调查，不要求开发者预先掌握。
- 解释、带读和读者文档服务于开发者的主动理解，不是每次任务的强制流程。
- `package:dashboard` 是技能包之外的可选只读辅助包，用于查看项目当前态、历史和 Git 派生状态；它不定义或执行技能工作。
- 技能不主动评价项目是否值得修改，不做无请求的项目治理或全面扫描，也不把每个请求扩张为全面审计或教学。
...
- `cs-explain` 拥有当前项目的即时解释，并按问题选择文字、图、树、伪代码、diff 或临时 HTML；`cs-learn` 只在当前对话中用项目代码、测试和必要的一手资料带读并验证理解；`cs-docs` 仅在用户要求长期材料时按 `skills/cs-docs/references/flow-doc.md` 写独立读者流程文档。
