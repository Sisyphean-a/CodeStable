---
name: cs-goal
description: "Pursue an explicit terminal objective autonomously until fresh evidence proves completion or a true external blocker repeats. Invoke explicitly with a goal and acceptance evidence."
...
2. **Restore.** On every continuation, compare each requirement with current files, command output, runtime behavior, and external state. Choose the highest-value unmet requirement. Complete when the next attempt is tied to one visible gap.
...
Stop as blocked only when the same external blocker has prevented progress in three consecutive goal turns and the exact user or external action required is known. Persist only the harness's minimal resume state; do not create a project goal package or iteration reports.

## Completion

Fresh evidence proves every terminal requirement and no required work remains, or the harness records a qualifying repeated external blocker with a precise recovery action.
